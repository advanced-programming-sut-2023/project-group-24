...
import view.menus.Lobby.LobbyMainMenu;
import view.menus.Lobby.LobbyMenu;
import view.menus.login.*;
import view.menus.main.*;
...
public class AppController {
    ...
    public AppController(Stage stage) {
        try {
            ...
            this.lobbyDatabase = new LobbyDatabase();
    ...
    private void receivedLobbyDatabase(Packet packet) {
        GsonBuilder gsonBuilder = new GsonBuilder();
        gsonBuilder.setPrettyPrinting();
        Gson gson = gsonBuilder.create();

        lobbyDatabase = gson.fromJson(packet.getValue(), LobbyDatabase.class);
    ...
    private void handleInputs(Packet packet) {
        ...
        if (packet.getTopic().equals("LobbyDatabase") && packet.getSubject().equals("lobbyDatabase")) receivedLobbyDatabase(packet);
        if (packet.getTopic().equals("lobbyDatabase")) handleLobbyCommands(packet, gson);
        if (packet.getTopic().equals("database")) handleDatabaseCommand(packet);
        if (packet.getTopic().equals("chat")) handleChatCommand(packet);
        if (packet.getTopic().equals("friend")) handleFriendCommand(packet);
    }

    private void handleLobbyCommands(Packet packet, Gson gson) {
        switch (packet.getSubject()) {
            case "addLobby":
                Lobby lobby = gson.fromJson(packet.getValue(), Lobby.class);
                lobbyDatabase.addLobby(lobby.getUsers().get(0), lobby.getCapacity(), lobby.getId(), lobby.isPrivateLobby());
                break;
            case "removeLobby":
                lobbyDatabase.removeLobby(Integer.parseInt(gson.fromJson(packet.getValue(), String.class)));
                break;
            case "changeAdmin":
                int index = Integer.parseInt(gson.fromJson(packet.getValue(), String.class));
                lobbyDatabase.getLobbies().get(index).changeAdmin();
                break;
            case "addUser" :
                User user = database.getUserByUsername(packet.getArgs()[0]);
                lobbyDatabase.getLobbies().get(Integer.parseInt(gson.fromJson(packet.getValue(), String.class))).addUser(user);
                startGame(lobbyDatabase.getLobbies().get(Integer.parseInt(gson.fromJson(packet.getValue(), String.class))));
                break;
            case "changeState" :
                lobbyDatabase.getLobbies().get(Integer.parseInt(gson.fromJson(packet.getValue(), String.class))).changeState();
                break;
            case "removeUser" :
                int index1 = Integer.parseInt(gson.fromJson(packet.getValue(), String.class));
                lobbyDatabase.getLobbies().get(index1).removeUser(packet.getArgs()[0]);
                if (lobbyDatabase.getCurrentLobby().getSize() == 0)
                    lobbyDatabase.removeLobby(index1);
                else lobbyDatabase.getCurrentLobby().changeAdmin();
                break;
        }
    }
    ...
    public void acceptRequest(Friendship friendship) {
        User user = database.getUserByUsername(friendship.getAccepterName());
        for (Friendship friend : user.getFriends()) {
            if (friend.equals(friendship))
    ...
    private void handleFriendCommand(Packet packet) {
        GsonBuilder gsonBuilder = new GsonBuilder();
        gsonBuilder.setPrettyPrinting();
        Gson gson = gsonBuilder.create();
        Friendship friendship = gson.fromJson(packet.getValue(), Friendship.class);
        switch (packet.getSubject()) {
            case "send":
    ...
    public void run(MenusName currentMenu) throws Exception {
        switch (currentMenu) {
            ...
            case CHAT_MENU:
                ...
                break;
            case LOBBIES_MENU:
                LobbyMainMenu lobbyMainMenu = new LobbyMainMenu(this);
                lobbyMainMenu.start(stage);
                break;
            case LOBBY_MENU:
                LobbyMenu lobbyMenu = new LobbyMenu(this);
                lobbyMenu.start(stage);
                break;
            case FRIENDSHIP:
                FriendShipMenu friendShipMenu = new FriendShipMenu(this);
                friendShipMenu.start(stage);
                break;
        }
    }
    ...
    public Controller getControllerForMenu(ControllersName name, Control control) {
        ...
        switch (name) {
            ...
            case LOBBY_CONTROL:
                return new LobbyController(inputOutputHandler, lobbyDatabase ,currentUser);
            case CHAT:
                return new ChatController(inputOutputHandler, database, chatDatabase, currentUser);
            case FRIEND:
                return new FriendController(inputOutputHandler, database, currentUser);
        }
        return null;
    }
    ...
    public void createRoom() {
        ChatController controller = new ChatController(inputOutputHandler, database, chatDatabase, currentUser);
        int n = lobbyDatabase.getCurrentLobby().getSize();
        String[] args = new String[n + 1];
        args[0] = lobbyDatabase.getCurrentLobby().getId();
        for (int i = 1; i < args.length; i++) {
            args[i] = lobbyDatabase.getCurrentLobby().getUsers().get(i - 1).getUsername();
        }
        controller.newRoom(args);
    }
}
