...
public class GameController {
    ...
    public void nextTurn(KingdomController kingdomController, MapGame mapGame) {
    ...
    private boolean checkNeighbor(Army army, Building building) {
        ...
        return Math.abs(changeX) < army.getArmyType().getRange() && Math.abs(changeY) < army.getArmyType().getRange();
    }
    ...
    private void moveUnits() {
        ...
        HashMap<Army, ArrayList<Cell>> path = new HashMap<>();
        for (Army army : currentKingdom.getArmies()) {
            path.put(army, new ArrayList<>());
            ...
            for (int i = 0; i < army.getArmyType().getSpeed(); i++) {
                ...
                armies.add(army);
                army.moveArmy();
                path.get(army).add(army.getLocation());
        ...
        mapGame.changeLocation(armies, path);
    }
        ...
        if (army.getPatrol() != null) {
            if (army.getPatrol().getObject1().equals(army.getLocation())) {
                ...
                if (pathFinder.search(new Pair<>(army.getPatrol().getObject2().getX(),
                    ...
                    army.setPath(new ArrayList<Cell>());
